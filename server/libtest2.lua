...
do -- Load items
    ...
    DropdownWindow.Position = UDim2.new(0, 100, 0, 100)
    ...
    ColorPickerWindow.Position = UDim2.new(0, 100, 0, 100)
...
local function getUIScale()
    local success, vp = pcall(function() return camera.ViewportSize end)
...
local function scaledSize(x, y)
    local s = getUIScale()
    return math.floor(x * s), math.floor(y * s)
end

local function mobileScale(value)
    if isMobile then
        return math.floor(value * math.max(1.0, 1.3 * (1 / getUIScale())))
    end
    return value
end
    ...
    UserInputService.InputBegan:Connect(function(inputObject)
        if inputObject.UserInputType == Enum.UserInputType.MouseButton1
            ...
            if inputObject.UserInputType == Enum.UserInputType.Touch then
                mouse.touchPosition = inputObject.Position
            end
...
local function getMouse()
    -- Try touch position first for mobile
    if mouse.touchPosition then
        return Vector2.new(mouse.touchPosition.X, mouse.touchPosition.Y - 36)
    end
...
local function findBrowsingTopMost()
    local copy = { }
    for i, v in next, windowHistory do
        if isBrowsing(i) or (isMobile and isTouchingWindow(i)) then
            copy[i] = v
        end
    end
...
local dragger = {} do
    ...
    function dragger.new(frame)
        ...
        -- Touch-based enter detection for mobile
        if isMobile then
        ...
        mouse.InputBegan:Connect(function()
            ...
            if topMost == frame or (isMobile and isTouchingWindow(frame)) then
                if (not colorpicking) and (not sliding) and not isDragging and (rawget(draggerCache, frame) or (isMobile and isTouchingWindow(frame))) then
                    isDragging = true
...
local library library = {
    ...
    new = function(options)
        ...
        local defaultWidth = isMobile and 260 or 300
        local defaultHeight = isMobile and 180 or 200
        ...
        -- Clamp position to screen on mobile
        if isMobile then
            local posX = options.position.X.Offset
            local posY = options.position.Y.Offset
            local maxX = math.max(10, viewportSize.X / getUIScale() - options.size.X - 20)
            local maxY = math.max(10, viewportSize.Y / getUIScale() - options.size.Y - 60)
            posX = math.clamp(posX, 10, maxX)
            posY = math.clamp(posY, 10, maxY)
            options.position = UDim2.new(0, posX, 0, posY)
        end
        ...
        function self.new(tabOptions)
            ...
            local types = { } do
                ...
                function types.dropdown(dropdownOptions)
                    ...
                    do -- search bar
                        ...
                        local clickOutConn = UserInputService.InputBegan:Connect(function(inputObject)
                            if (inputObject.UserInputType == Enum.UserInputType.MouseButton1 or inputObject.UserInputType == Enum.UserInputType.Touch) and canType then
                                local mousePos = inputObject.UserInputType == Enum.UserInputType.Touch and Vector2.new(inputObject.Position.X, inputObject.Position.Y) or UserInputService:GetMouseLocation()
                ...
                function types.number(numberOptions)
                    ...
                    textBox.TextColor3 = Color3.new(1, 1, 1)
                    textBox.TextSize = 13
                    textBox.ZIndex = inner.ZIndex + 1

                    local textChangeConn = textBox:GetPropertyChangedSignal("Text"):Connect(function()
                        local txt = textBox.Text:match("^%-?%d*%.?%d*$") and textBox.Text or textBox.Text:gsub("[^%d%.%-]", "")
